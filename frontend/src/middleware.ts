 ...
 *
 * This build is published for exactly one audience: someone who tapped
 * "Open private chat" in WhatsApp. Everything else the app serves — its own
 * sign-in, registration, calls and status pages — is not part of that, and
 * on this deployment it is not even functional: there is no NEXT_PUBLIC_API_URL
 * here, so the login form posts to this host and gets a 404. A broken sign-in
 * page on a link handed to customers is worse than no page at all, and it is
 * the one surface an outsider poking at the root URL will find.
 *
